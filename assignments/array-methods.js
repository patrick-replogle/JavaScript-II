...
let fullName = [];
let newArray = runners.forEach(function(currentValue){
...
// Problem 1
//Use for each to help the event director create a list of the companies that will be sponsoring the event.  Then sort the companies alphabetically so it's easy to locate each within the spreadsheet. 
let sponsors = [];
let companies = runners.forEach(function(currentValue){
  sponsors.push(currentValue.company_name);
});
console.log(sponsors.sort());

// Problem 2
//Use map to return new array of runner's email so the event director can spam all of the runners a last minute update at the same time.
let emails = runners.map(function(currentValue){
...
//Use filter to return new array of runners who wear small & medium shirts//
let smallAndMedium = runners.filter(function(currentValue){
    return currentValue.shirt_size == "S" || currentValue.shirt_size == "L";
  });
  console.log(smallAndMedium)